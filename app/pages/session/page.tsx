...
const sessionPage = () => {
    ...
    if (!token) {
      ...
      const fetchCurrentGuess = async () => {
        try {
          ...
          setIsLoadingGuess(false); // Arrête le chargement une fois la requête terminée
        }
      };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-gray-50 p-4">
      ...
      {isLoadingGuess ? (
        <p className="text-gray-500">Vérification si vous avez déjà fait un pronostic...</p>
      ) : (
      ...
      <div className="text-slate-500 hover:text-black hover:cursor-pointer mt-2" onClick={handleLogout}>
        Se déconnecter
      </div>
    </main>
  );
