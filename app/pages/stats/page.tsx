...
const StatsPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchRankings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(data.rankings));
                    localStorage.setItem(LAST_UPDATE_KEY, new Date().toISOString().split("T")[0]); // Sauvegarde la date du jour
        ...
        if (lastUpdate === today) {
            // Si la mise à jour a déjà été faite aujourd'hui, charger les données du localStorage
